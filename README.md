...
- 红色故事支持简明/完整版阅读、关怀模式及真实音频的暂停续播、进度拖动和倍速（设备支持时）；现有录音待采集，缺少录音时不会显示可播放内容。
...
发布体检会自动检查页面、云函数、腾讯坐标、本地素材、包体预估、强特征密钥、私人文件和参赛材料。`FAIL` 必须修复；`WARN` 表示仍需在提交前完成的人工或外部条件。2026-09-20 本机15个测试脚本全部通过，静态回归检查127个源码文件，发布检查结果为21 PASS / 4 WARN / 0 FAIL。

当前相册已独立分包：主包静态预估约0.28 MB，`assets`相册分包约1.63 MB，总体约1.91 MB；最终结果以微信开发者工具包体分析为准。

## 团队协作

请先接受仓库邀请，再按 [团队协作说明](CONTRIBUTING.md) 各建功能分支、测试并发起 Pull Request。GitHub 保存已推送的版本，不会实时同步各人的本地文件。当前功能入口以 README 和代码为准；已有参赛 PDF 尚未同步 2026-09-20 的首页与故事播放器调整。
