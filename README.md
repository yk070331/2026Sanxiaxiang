...
- 相册新增5张 Wikimedia Commons 开放授权的井冈山区域照片，记录摄影者、CC BY-SA 4.0许可和原作链接；共21张可展示图片。默认隐藏12项待采集素材，可展开查看；区域照片不替代乔林实景。
...
发布体检会自动检查页面、云函数、腾讯坐标、本地素材、包体预估、强特征密钥、私人文件和参赛材料。`FAIL` 必须修复；`WARN` 表示仍需在提交前完成的人工或外部条件。2026-09-20 本机17个测试脚本全部通过，静态回归遍历141个小程序/云函数文件（含素材），发布检查结果为21 PASS / 4 WARN / 0 FAIL。

当前相册已独立分包：主包静态预估约1.58 MB（含合成语音及新增开放图片），`assets`相册分包约1.63 MB，总体约3.21 MB；最终结果以微信开发者工具包体分析为准。
