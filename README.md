...
- 首页常驻八个入口：红色故事、路线推荐、研学打卡、知识闯关、我的下一站、研学证书、实景相册、村庄详情；底部直接切换关怀模式、提交游客纠错或进入可信与隐私中心。
- 从首页继续最近选择的研学路线；证书仍需完成该路线全部旧址打卡，下一站仍遵守坐标核验限制。切换短路线打卡不会清除其他站点进度。
- 红色故事支持简明/完整版阅读、关怀模式和六段普通话合成导览（每段约42–46秒），可暂停续播、拖动进度和倍速播放（设备支持时）。页面与音频开头均标注“AI 合成讲解，非真人口述”；真实口述录音仍待采集。
- 相册共34张可展示图片：含5张 Wikimedia Commons 开放授权照片及13张用户补充照片。新增照片已归入对应分类，也可点“补充照片”集中查看；默认隐藏7项待采集素材。黄洋界与乔林乡党支部旧址详情使用对应介绍图，点击查看组照。未知地点照片保留待核验说明，不据此生成导航坐标。
- 全域地图、点位搜索、类别筛选、当前位置与腾讯地图导航。
- 乔林村委会/游客入口、乔林水库可达入口/大坝使用腾讯地图核验坐标。
- 村庄详情和路线页直接选择30分钟初心线、60分钟研学线、半日乡村线；研学页顶部还能切换完整线（约2小时），保留已打卡站点。展示距离均为待实测规划参考。
...
村内六处旧址仍需逐点取得腾讯地图分享位置或现场采集坐标。在核验完成前，小程序不会用推测坐标提供导航或虚构路线连线。无坐标按钮显示“到访指引”，用户确认后可先导航到乔林村委会集合点；该点不是旧址位置。路线起点统一为已核对的村委会位置，原古樟树不再作为无坐标起点。定位失败区分授权、隐私配置、接口权限与系统定位问题，地图打开失败可复制位置。
...
miniprogram/
  ...
  contributions/        补充照片分包：13张完整图片及分组预览页
  utils/
    data.js             本地已审核核心数据
...
发布体检会自动检查页面、云函数、腾讯坐标、本地素材、包体预估、强特征密钥、私人文件和参赛材料。`FAIL` 必须修复；`WARN` 表示仍需在提交前完成的人工或外部条件。2026-09-20 本机18个测试脚本全部通过，静态回归遍历173个小程序/云函数文件（含素材），发布检查结果为22 PASS / 4 WARN / 0 FAIL。

当前主包静态预估约1.84 MB（含语音及图片缩略图），`assets`相册分包约1.63 MB，`contributions`补充照片分包约1.75 MB，总体约5.22 MB；最终结果以微信开发者工具包体分析为准。

## 团队协作

请先接受仓库邀请，再按 [团队协作说明](CONTRIBUTING.md) 各建功能分支、测试并发起 Pull Request。GitHub 保存已推送的版本，不会实时同步各人的本地文件。当前功能入口以 README 和代码为准；已有参赛 PDF 尚未同步 2026-09-20 的首页、相册与合成讲解调整。

图片署名见 [开放图片署名与许可](参赛材料/开放图片署名与许可.md)；语音稿与制作说明见 [合成讲解说明](参赛材料/合成讲解说明.md)。新增图片继续按原图许可使用；这不表示原有团队素材也获得了相同许可。模拟器验证记录见 [图片与语音验收](参赛材料/图片与语音验收.md)。

13张用户补充照片的分类与核验说明见 [补充照片接入说明](参赛材料/补充照片接入说明.md)，原片与发布图片的校验值见 [用户补充照片记录](参赛材料/用户补充照片记录.json)。这些用户素材不属于上述 CC BY-SA 图片集合。

## 当前证据边界

已经完成本地功能实现、云函数逻辑测试、页面静态回归、地图分享位置核对、云环境启动容错、发布体检和正式参赛PDF生成。提交体验版前仍需完成：

- 模拟器已完成编译、开放图片分类/大图展示和合成语音播放验证；手机预览与实际上传包体检查仍待完成。
- 微信云环境部署及三个角色账号的权限验证。
